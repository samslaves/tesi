...
def H_ring(J, Jp, b):
    H = J*dot(1,2) + Jp*(dot(2,3) + dot(3,1))
    for s in (1,2,3):
        H = H + b*op(s,Z)
    return H

# indice base computazionale |site1 site2 site3>, bit=0 -> up (Z=+1), bit=1 -> down (Z=-1)
def ket(bits):
...
# otto stati di Kambe, da derivazione_stati_kambe_trimero.pdf (Tabella 1)
states = {
 ("A", 1.5): [(1.0,'000')],
 ("A", 0.5): [(1/s3,'001'),(1/s3,'010'),(1/s3,'100')],
 ("A",-0.5): [(1/s3,'011'),(1/s3,'101'),(1/s3,'110')],
 ("A",-1.5): [(1.0,'111')],
 ("B", 0.5): [(np.sqrt(2/3),'001'),(-1/s6,'010'),(-1/s6,'100')],
 ("B",-0.5): [(1/s6,'011'),(1/s6,'101'),(-np.sqrt(2/3),'110')],
 ("C", 0.5): [(1/s2,'010'),(-1/s2,'100')],
 ("C",-0.5): [(1/s2,'011'),(-1/s2,'101')],
}

QN = {"A":(1,1.5), "B":(1,0.5), "C":(0,0.5)}  # (S12, S)

def E_teor(mult, M, J, Jp, b):
    S12, S = QN[mult]
    return 2*(J-Jp)*S12*(S12+1) + 2*Jp*S*(S+1) - 3*J - 1.5*Jp + 2*b*M
