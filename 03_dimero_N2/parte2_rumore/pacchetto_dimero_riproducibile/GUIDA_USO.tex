...
\code{dati/} & dati intermedi (\code{.npz}) generati da 01\_ e 02\_\\
\code{figure/} & 18 figure finali, in PDF e PNG (14 aggiunte in questo aggiornamento, Sez.~\ref{sec:aggiornamento})\\
\code{genera\_figure/} & 8 script (4 originali + 4 aggiunti)\\
...
\code{03\_correlatore\_da\_vqe.py} & presente ma NON eseguito da \code{esegui\_tutto.py} --- problema aperto, Sez.~\ref{sec:aggiornamento}\\
...
aggiunte 14 figure per allineare il pacchetto al contenuto reale dei
...
$\approx0.82$ di PMA a $K=1$ (Sez.~\ref{sec:pma-k1} sotto).
...
\begin{warnbox}{Problema aperto, non risolto: fedeltà di PMA base a
$b/J=-0.18$, $D/J=1$}
Il Documento 2 (\S6, tabella di confronto) riporta $\mathcal F=0.922857$
per l'ansatz PMA base (1 parametro) a questo punto di lavoro. La
ricostruzione più diretta di questo ansatz
(\code{ansatz\_PMA\_W}/\code{make\_pma\_ansatz}, verificate identiche
fra loro) raggiunge invece un tetto di $\mathcal F=0.851664$, verificato
con: scan fine su $\theta$ (non un problema di ottimizzatore); minimizzazione
dell'energia invece della fedeltà diretta (stesso risultato); scambio
della convenzione qubit (nessuna differenza); direzione del CNOT e
qubit di applicazione della $R_y$ scambiati (nessuna variante supera
$\approx0.85$); $D=0.2$ invece di $D=1$ nello stesso punto ($\mathcal
F=0.990$, ancora diverso da $0.922857$); segno di $D$ invertito
(identico, il sistema è simmetrico per $D\to-D$); segno di $b$
invertito (identico, simmetrico anche per $b\to-b$); $b=0$ esatto
($\mathcal F=0.853553$, praticamente indistinguibile da $b=-0.18$ ---
non è un problema di precisione su $b$). Il valore di $D$ che
darebbe davvero $0.922857$ è $D\approx0.62$--$0.63$: non un valore
``pulito'', non coincide né con $D=1$ né con $D=0.2$. Causa non
identificata dopo nove ipotesi testate --- probabilmente non
risolvibile senza accesso al codice/sessione originale che ha prodotto
il numero $0.922857$. \textbf{La figura \code{fig14\_correlatore\_da\_vqe}
non è stata prodotta} in attesa di risolvere questa discrepanza. Lo
script \code{03\_correlatore\_da\_vqe.py} esiste nel pacchetto ma non è
richiamato da \code{esegui\_tutto.py} per questo motivo.
\end{warnbox}
...
$D=0.2$'' mostra un calo netto di fedeltà (minimo $\approx0.82$, non
...
guida è stato ricontrollato. Verificato di nuovo dopo l'aggiornamento
di Sez.~\ref{sec:aggiornamento}: \code{exit code 0}, 18 figure prodotte
(esclusa \code{fig14\_correlatore\_da\_vqe}, non ancora prodotta per il
problema aperto documentato sopra).
\end{okbox}
