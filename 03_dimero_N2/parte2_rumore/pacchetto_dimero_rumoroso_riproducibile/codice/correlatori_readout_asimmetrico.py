...
RUOLO NELL'INSIEME DEL PACCHETTO: e' il modulo dell'estensione readout
asimmetrico -- generalizza correlatori_rumorosi_dimero.py (di cui
importa build_noisy_correlator_circuit e ancilla_z_gate_noisy) dal
readout simmetrico (1-2p) a uno asimmetrico (trasformazione affine).
Non ridefinisce il circuito: la differenza rispetto al modulo base sta
tutta nella funzione di correzione applicata DOPO la simulazione del
rumore di gate, coerente con la scelta di applicare il readout
analiticamente gia' stabilita li'.
"""
import numpy as np
...
def correzione_readout(z_ideale, p01, p10):
    ...
    Ruolo nel modulo: e' la funzione che generalizza la correzione
    (1-2p) di correlatori_rumorosi_dimero.py -- usata da
    correlator_rumoroso_asimmetrico() al posto della moltiplicazione
    semplice.
    Ruolo nell'insieme: il limite p01=p10=p deve ridurre esattamente
    alpha=1-2p, beta=0, cioe' la formula simmetrica gia' verificata --
    e' il primo controllo fatto nel blocco __main__, prima di guardare
    il caso genuinamente asimmetrico.
    """
    alpha = 1.0 - p01 - p10
    beta = p10 - p01
...
def correlator_rumoroso_asimmetrico(i, alpha, j, beta, t, N, J, b, D,
    ...
    riusa build_noisy_correlator_circuit() e ancilla_z_gate_noisy() di
    correlatori_rumorosi_dimero.py identici (nessuna differenza nel
    circuito o nel rumore di gate), applicando poi correzione_readout()
    invece della moltiplicazione (1-2p).
    Ruolo nell'insieme: e' la funzione chiamata da
    trova_N_star_asimmetrico() per trovare N* al variare dello split
    p01/p10 -- il risultato centrale dell'estensione (N*=5 invariato)
    nasce dal confrontare le sue uscite a diversi valori di p01, p10.
    """
...
def trova_N_star_asimmetrico(vqe_params, t, N_grid, J, b, D, noise_model,
    ...
    Ruolo nel modulo: e' la funzione di ricerca del massimo usata da
    OGNI controllo del blocco __main__ (limite simmetrico, test
    illustrativo, stress test a rapporti estremi) -- l'unico punto del
    file dove si cerca effettivamente un N*.
    Ruolo nell'insieme: e' l'analogo, per il readout asimmetrico, di
    trova_N_star() in scan_parametri_rumore_dimero.py (che pero' lavora
    sulla fedelta' di Trotter, non sul correlatore) e di
    N_star_correlatore() in verifica_passo5_noise_aware.py (che lavora
    sul correlatore ma con readout simmetrico) -- tre varianti dello
    stesso pattern "cerca il massimo su una griglia di N", ciascuna per
    una combinazione diversa di osservabile e tipo di readout.
    """
    vals = []
    for N in N_grid:
...
def readout_error_asimmetrico_aer(p01, p10):
    ...
    Ruolo nel modulo: costruisce l'oggetto ReadoutError di Qiskit Aer
    usato da correlator_montecarlo_asimmetrico() per il secondo cammino
    di calcolo -- la controparte "vera" (shot finiti) della correzione
    analitica applicata da correzione_readout().
    Ruolo nell'insieme: e' l'unico punto di tutto il pacchetto dove
    viene costruita una matrice di confusione ASIMMETRICA per Aer (il
    modello di rumore base, noise_model_dimero.py, costruisce sempre e
    solo ReadoutError simmetrici).
    """
...
def correlator_montecarlo_asimmetrico(i, alpha_op, j, beta_op, t, N, J, b, D,
    ...
    Ruolo nel modulo: e' il controllo di verifica indipendente per
    correlator_rumoroso_asimmetrico() -- misura a shot finiti (come
    avverrebbe davvero su hardware) invece di applicare la formula
    analitica, per confermare che la formula sia corretta e non solo
    plausibile.
    Ruolo nell'insieme: stesso ruolo, per il caso asimmetrico, del
    cross-check Monte Carlo gia' presente in
    validate_correlatori_rumorosi_dimero.py per il caso simmetrico --
    due cammini di calcolo indipendenti (analitico via operatore
    densita', Monte Carlo via conteggi a shot finiti) per lo stesso
    risultato fisico.
    """
