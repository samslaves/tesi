...
\textbf{Verifiche.} Otto self-test base più tre self-test DM, identici nello schema a
quelli dell'anello. Rieseguiti in questa sessione (versione più recente, caricata con
le funzioni DM già integrate): tutti superati a precisione macchina, incluso il
self-test aggiuntivo di coerenza fra \texttt{exact\_sweep\_dm} e
\texttt{ground\_state\_projector\_dm} (assente nella prima versione del file
disponibile a inizio sessione, poi sostituita da quella più recente e corretta).
...
\item \texttt{\_self\_test\_dm()} — verifica la simmetria $P_{13}$ del termine DM
simmetrico e la sua rottura nel caso non simmetrico.
\end{itemize}
