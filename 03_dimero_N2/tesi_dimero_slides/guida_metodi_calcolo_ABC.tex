...
usa \texttt{Statevector} sull'ansatz con i parametri assegnati, per ogni
fidelity riportata, e \texttt{np.linalg.eigh(H)} per il riferimento esatto.
...
Fidelity ansatz base, punto per punto & \textbf{B} vs \textbf{A} \\
...
Il documento dove B e C avevano coinciso, verificato, prima di sostituire C
con il circuito vero per intero. Verificato in \texttt{trotter\_dimero.py}
e \texttt{fig\_doc3\_circuito\_vero.py} / \texttt{fig\_doc3\_due\_ansatz\_vero.py}.
...
$\media{S_z}(t)$ esatto vs circuito ($N{=}2,5,20$) & esatta: \textbf{A}.
Circuito: \textbf{B} --- passo transpilato in porte native, composto $N$
volte \\
Convergenza dell'errore (log-log) & \textbf{B} per tutti i 10 valori di $N$,
incluso l'operatore completo via \texttt{Operator()} \\
Errore a $t=6$ (tabella) & stessi dati della figura precedente, \textbf{B} \\
...
``Due ansatz'' (PMA base/esteso, con/senza circuito) & stato VQE: \textbf{B}
(Doc.\ 2). Evoluzione esatta: \textbf{A}. Evoluzione ``circuito $N{=}20$'':
\textbf{B} --- \texttt{Statevector(psi0).evolve(circuito)} \\
\bottomrule
\end{tabular}
\end{center}

\verifica{Fino a una sessione precedente, tutte le righe ``circuito'' sopra
usavano invece la matrice compatta C (\texttt{U\_trotter\_mat}), verificata
identica a B a precisione macchina (scarto $2.4\times10^{-13}$ su tutta la
griglia di $N$, da $1$ a $512$) prima di essere sostituita per intero: ogni
numero pubblicato è rimasto lo stesso, verificato di nuovo dopo la
sostituzione, non solo atteso.}

\section{Documento 4 --- Correlatori}

Verificato in \texttt{fig\_doc4.py}, \texttt{fig\_doc4\_spettro.py},
\texttt{fig\_doc4\_vqe.py} e nello script del circuito vero per lo scan, e
nel modulo \texttt{circuito\_correlazioni\_dimero.py}.
...
Segnale ricco/monocromatico & \textbf{A} pura \\
...
Scan 36 combinazioni con stato VQE (PMA base/esteso) & stato: \textbf{B}.
Dinamica: \textbf{B} --- circuito vero completo, stesso schema della riga
precedente, ripetuto su tutte le $36$ combinazioni \\
\bottomrule
\end{tabular}
\end{center}

\verifica{Fino a una sessione precedente, lo scan sulle $36$ combinazioni con
stato VQE usava l'esponenziale diretto (\textbf{A}) al posto del circuito,
per tempo di calcolo. Rifatto con il circuito vero: per il PMA base i numeri
restano gli stessi entro l'incertezza della griglia temporale (scarto medio
$0.34$, invariato); per il PMA esteso il numero \textbf{cambia
sostanzialmente} --- da rumore numerico trascurabile ($10^{-7}$--$10^{-8}$,
un artefatto del confronto fra due evoluzioni entrambe esatte) a un errore
di Trotter reale ($0.011$--$0.032$), perché un circuito vero non può
realizzare l'esponenziale esatto che il calcolo diretto usava implicitamente
anche per lo stato quasi perfetto del PMA esteso.}
...
--- il vero errore di Trotter, prima nascosto. Con l'ansatz base lo scarto
...
\item \textbf{Documento 3}: le curve ``circuito'' nei grafici principali
sono ora genuinamente B (passo di Trotter transpilato in porte native,
composto $N$ volte) --- prima erano C, verificate identiche a precisione
macchina e poi sostituite per intero.
\item \textbf{Documento 4}: la maggior parte delle figure (scan al punto
esatto, ricco/monocromatico, spettro, ricostruzione) è A pura, mai un circuito ---
quella parte non riguarda il VQE. Ovunque compaia uno stato del VQE (la
figura di validazione, il correlatore singolo, ed entrambi gli scan
sistematici sulle 36 combinazioni, PMA base ed esteso) è ora B: circuito
vero, ancilla inclusa. Per il PMA esteso questo ha cambiato la conclusione
numerica, non solo il metodo: da rumore trascurabile a un errore di
Trotter reale, seppure piccolo.
\end{itemize}
