...
\begin{panelbox}{Ansatz base --- PMA semplificato}
...
\emph{Attenzione al nome:} questo $W_{ij}(\theta)$ è il blocco base usato
in questo progetto (due CNOT attorno a un'unica $R_y$) --- \textbf{non}
coincide col gate $W_{ij}(\theta)=e^{-i\theta\,\mathbf s_i\cdot\mathbf
s_j}=R_{xx}(\theta/2)R_{yy}(\theta/2)R_{zz}(\theta/2)$ di Crippa et al.\
2021 (tre porte a due qubit, genuinamente $M$-conservante su tutto lo
spazio). Il blocco qui sopra è più economico (due CNOT contro tre porte
a due qubit) ma, verificato esplicitamente, \textbf{non} conserva $M$ su
tutto lo spazio a due qubit: applicato a $|00\rangle$ o $|11\rangle$ li
mischia fra loro ($|00\rangle\to\cos\frac\theta2|00\rangle+
\sin\frac\theta2|11\rangle$); conserva $M$ solo quando il suo ingresso è
già nel settore $\{|01\rangle,|10\rangle\}$ --- il caso del ramo
$B/J<2$ qui sotto, non quello di $B/J\geq2$.
...
locale $(|01\rangle-|10\rangle)/\sqrt2$ --- qui il blocco ruota
genuinamente dentro il settore $M=0$, nessuna approssimazione;
\item $B/J\geq2$ (GS = $|11\rangle$, $M=-1$): si parte da $|11\rangle$
--- qui il blocco potrebbe in linea di principio mescolare verso
$|00\rangle$, ma l'ottimo resta a $\theta\approx0$ (il target è già lì),
quindi in pratica non serve muoversi fuori da quel settore.
\end{itemize}
Per il dimero ($N=2$, un legame, $K=1$): \textbf{1 solo parametro}.
\end{panelbox}
...
2 & HA & 0.2 & 6\\
3 & Ansatz base (PMA semplificato, $K=1$) & 0 & 1\\
4 & Ansatz base & 0.2 & 1\\
\bottomrule
\end{tabular}
\end{center}
...
interna del blocco base $W_{ij}(\theta)$ (non il $W_{ij}$ di Crippa,
vedi nota sopra), che rivela \emph{come} un solo
