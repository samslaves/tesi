...
def w_block(qc, theta, q0, q1):
    ...
    coerente col legame $M$-preservante gia' usato sul dimero.

    RUOLO NEL MODULO: applicato ai tre legami dell'anello in
    pma_2q_trimer_exact. RUOLO NELL'INSIEME: 2 CNOT per legame, 6 CNOT
    totali per l'ansatz W-2q.6 -- costo verificato in Stadio 1 di Parte 2."""
...
def pma_2q_trimer_exact(nparam, block):
    """Costruisce l'ansatz PMA-2q generico per il trimero (3 qubit): un
    blocco per ciascuno dei tre legami, poi eventuali strati aggiuntivi
    di rotazioni locali RY se nparam > 3.

    RUOLO NEL MODULO: factory generica, usata con block=w_block per
    ottenere l'ansatz W-2q (RBS-2q si ottiene passando un altro block,
    non incluso in questo modulo). RUOLO NELL'INSIEME: w2q6_circuit()
    e' la specializzazione a 6 parametri (nparam=6) usata in tutta la
    pipeline, sia Parte 1 sia Parte 2."""
...
def w2q6_circuit():
    """Ansatz canonico per il VQE del trimero ad anello: W-2q a 6
    parametri (3 blocchi W, un ulteriore strato di RY locali).

    RUOLO NEL MODULO: specializzazione di pma_2q_trimer_exact.
    RUOLO NELL'INSIEME: importato in tutta la pipeline di Parte 2
    (Stadi 2-5) come punto d'ingresso unico per l'ansatz -- scelto su
    RBS-2q per la fedelta' esatta ($\\mathcal F=1$) sotto il termine DM,
    vedi log_decisioni.md."""
    return pma_2q_trimer_exact(6, w_block)


def bound_statevector(params):
    """Statevector del circuito w2q6_circuit() con i parametri assegnati.

    RUOLO NEL MODULO: usato dalla funzione di costo energy() sotto.
    RUOLO NELL'INSIEME: nessun uso diretto fuori da questo modulo (Parte
    2 valuta l'energia via simulatore a operatore densita', non
    statevector diretto)."""
...
def main():
    """Ottimizza il VQE con termine DM al punto di lavoro confermato
    (J=1, J'=0.4, b=b_c=2.4, D=0.15, mode 'B'): multistart COBYLA +
    polish L-BFGS-B, salva parametri ottimali e fidelity.

    RUOLO NEL MODULO: blocco __main__ effettivo (chiamato sotto
    if __name__=='__main__'). RUOLO NELL'INSIEME: produce
    w2q6_params_optimal.npz, l'input condiviso da tutta la pipeline di
    Parte 2 e dagli script di validazione dei correlatori di Parte 1."""
    ...
    def energy(params):
        """Funzione di costo VQE: energia di aspettazione <psi(params)|H|psi(params)>
        da statevector esatto (nessun rumore -- Parte 1, sistema chiuso)."""
