...
RUOLO NELL'INSIEME DEL PACCHETTO: e' il controllo obbligatorio per il
supporto ansatz_params di circuito_correlazioni_dimero.py -- verifica
che preparare lo stato tramite il circuito VQE reale (invece delle
ampiezze esatte via prepare_state) dia risultati consistenti, non
introduca permutazioni di qubit nascoste, e costi solo 1 CNOT in piu'
della preparazione esatta. Nota: importa BASIS_GATES da
noise_model_dimero.py -- una dipendenza verso un modulo concettualmente
di Parte 2 (rumore), incluso in questo pacchetto SOLO per questa
costante (la base di gate {rz,sx,x,cx} usata per transpilare), non per
la sua funzionalita' di modellazione del rumore.
"""
import numpy as np
