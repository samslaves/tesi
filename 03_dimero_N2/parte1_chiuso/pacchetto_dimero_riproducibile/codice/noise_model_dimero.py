...
RUOLO NELL'INSIEME DEL PACCHETTO: e' il modulo "cerniera" fra Parte 1
(sistema chiuso, nessun rumore) e Parte 2 -- il primo file di tutta la
pipeline di rumore, e l'unico che costruisce l'oggetto NoiseModel di
Qiskit Aer. Ogni altro modulo di Parte 2 (VQE sotto rumore, Trotter
sotto rumore, correlatori sotto rumore, scan sui parametri, ed
entrambe le estensioni) importa build_noise_model() da qui invece di
costruire un proprio NoiseModel indipendente -- garantisce che "rumore"
significhi sempre, in tutto il pacchetto, esattamente questi due canali
a questi valori (o a quelli esplicitamente scansionati), non una
replica generica e potenzialmente incoerente da un modulo all'altro.
"""
...
def eps_to_lambda(eps_1q, eps_2q):
    ...
    Ruolo nel modulo: e' il passaggio di conversione usato internamente
    da build_noise_model() -- separato in una funzione propria perche'
    la formula (e la distinzione fra eps da randomized benchmarking e
    lambda di Aer) e' un punto facile da sbagliare, quindi isolato e
    testabile a parte.
    Ruolo nell'insieme: e' l'unico punto di tutto il pacchetto dove
    avviene questa conversione -- se la formula cambiasse, cambierebbe
    coerentemente per tutta la pipeline che passa da build_noise_model().
    """
    lam_1q = 2.0 * eps_1q
...
def build_noise_model(eps_1q=EPS_1Q_REF, eps_2q=EPS_2Q_REF,
    ...
    Ruolo nel modulo: e' la funzione pubblica del file -- tutto il resto
    (costanti di calibrazione, eps_to_lambda) esiste in funzione di
    questa singola chiamata.
    Ruolo nell'insieme: e' la funzione piu' importata di tutta la Parte 2
    del pacchetto -- ogni script che valuta un circuito "sotto rumore"
    chiama prima questa funzione per ottenere il NoiseModel da agganciare
    ad AerSimulator. Il fatto che accetti eps_1q/eps_2q/p_readout come
    parametri (non solo costanti fisse) e' cio' che rende possibile lo
    scan sui parametri di rumore (Passo 5) senza duplicare codice.
    """
