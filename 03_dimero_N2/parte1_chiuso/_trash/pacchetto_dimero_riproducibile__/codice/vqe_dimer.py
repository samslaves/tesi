...
RUOLO NELL'INSIEME DEL PACCHETTO: e' il modulo che GIUSTIFICA la scelta
dell'ansatz PMA fatta altrove (vqe_test2.py e, nel pacchetto rumoroso,
tutta la pipeline di Parte 2) -- confronta esplicitamente l'ansatz
"generico" (hardware-efficient) contro quello motivato dalla fisica del
problema, su un intero sweep di campo b e a due valori di D, non solo
al singolo punto di lavoro "test 2". E' il modulo con lo scope piu'
ampio del pacchetto: mentre vqe_test2.py ottimizza UN punto con UN
ansatz, questo ne ottimizza 80 (20 valori di b, 4 configurazioni
ansatz/D) per costruire il confronto completo.
"""
...
def make_ha_ansatz(reps=2):
    ...
    Ruolo nel modulo: e' una delle due famiglie di ansatz messe a
    confronto da run_vqe()/vqe_sweep() -- il termine di paragone
    "generico", senza informazione sulla struttura fisica del problema.
    Ruolo nell'insieme: stessa idea di ansatz_HA() in ansatz_dimero.py
    (libreria di illustrazione) -- qui e' la versione effettivamente
    usata per produrre i risultati del confronto HA-vs-PMA.
    """
    return n_local(
        num_qubits=2,
        rotation_blocks="ry",
...
def make_pma_ansatz(b, J=1.0, K=1):
    ...
    Ruolo nel modulo: e' la seconda famiglia di ansatz confrontata da
    run_vqe()/vqe_sweep() -- a differenza di make_ha_ansatz(), lo stato
    iniziale dipende dal regime fisico (b/J sopra o sotto la soglia di
    anticrossing 2), il che lo rende un ansatz "su misura" per il
    problema invece che generico.
    Ruolo nell'insieme: stessa costruzione di ansatz_PMA_W() in
    ansatz_dimero.py -- qui e' la versione usata per produrre i
    risultati del confronto, che nel resto del progetto motiva la scelta
    finale (PMA-2q.3 di vqe_test2.py, un'estensione di questa idea con
    piu' parametri).
    """
...
def _energy(params, ansatz, hamiltonian, estimator):
    ...
    Ruolo nel modulo: e' la funzione minimizzata da run_vqe() -- calcola
    l'aspettazione dell'Hamiltoniana sull'ansatz assegnato ai parametri
    dati, usando il primitive StatevectorEstimator di Qiskit 2.x.
    Ruolo nell'insieme: concettualmente identica all'obiettivo interno
    di vqe_multistart() in vqe_test2.py, ma scritta con il pattern
    "primitive" (StatevectorEstimator) invece che con Statevector diretto
    -- due stili API equivalenti, entrambi in uso nel progetto.
    """
...
def run_vqe(b, J=1.0, D=0.0, ansatz_type="HA", reps=2, K=1,
    ...
    Ruolo nel modulo: e' la funzione di ottimizzazione a un solo punto,
    chiamata ripetutamente da vqe_sweep() per costruire l'intero sweep
    in b. Include gia' il confronto con il benchmark esatto
    (exact_sweep() di dimer_exact.py) nello stesso dict di ritorno.
    Ruolo nell'insieme: e' l'analogo, per un ansatz generico (HA o PMA a
    K layer), di vqe_multistart() in vqe_test2.py (specializzata su
    PMA-2q.3) -- qui il multistart e' piu' semplice (solo L-BFGS-B da
    punti casuali, senza il polish COBYLA+L-BFGS-B a due stadi usato li').
    """
    ...
    fid    = float(np.abs(sv.conj() @ gs_exact)) ** 2  # |<exact|vqe>|^2 (convenzione Qiskit/state_fidelity, uniformata al resto del progetto)
...
def vqe_sweep(b_values, J=1.0, D=0.0, ansatz_type="HA", reps=2, K=1,
    ...
    Ruolo nel modulo: e' la funzione di orchestrazione chiamata dal
    blocco __main__ per ciascuna delle 4 configurazioni (HA/PMA x
    D=0/D=0.2) -- ripete run_vqe() su tutta la griglia di b, con un seed
    diverso per ciascun punto (seed+i) per evitare correlazioni
    artificiali fra ottimizzazioni vicine.
    Ruolo nell'insieme: il suo output alimenta direttamente plot_grid()
    per produrre la figura di confronto -- e' il livello di
    orchestrazione che trasforma "un singolo VQE" (run_vqe) in "un
    confronto sistematico su tutto il diagramma di fase" (questo
    pacchetto non ha un analogo diretto per il punto singolo "test 2",
    dove basta un'unica chiamata a vqe_multistart in vqe_test2.py).
    """
    results = []
...
def _plot_row(axes, results, J, row_label, color):
    """Disegna una riga della griglia 4x4 (energia, errore, fedelta',
    magnetizzazione per una configurazione ansatz/D).

    Ruolo nel modulo: e' la funzione di disegno di UNA riga -- chiamata
    4 volte da plot_grid() (una per configurazione HA/PMA x D=0/D=0.2),
    con colore diverso per ciascuna.
    Ruolo nell'insieme: puramente di presentazione -- non calcola nulla
    di nuovo, visualizza i risultati gia' prodotti da vqe_sweep().
    """
...
def plot_grid(all_results, J=1.0, base_path="vqe_dimer"):
    ...
    Ruolo nel modulo: e' la funzione pubblica di presentazione --
    orchestra le 4 chiamate a _plot_row() (una per configurazione) e
    salva la figura finale.
    Ruolo nell'insieme: e' l'output visivo che riassume l'intero
    confronto HA-vs-PMA -- la figura da cui si legge il motivo per cui
    il progetto adotta poi PMA (e non HA) come base per l'ansatz finale
    usato ovunque nella pipeline (vqe_test2.py e, nel pacchetto
    rumoroso, tutta la Parte 2).
    """
    import matplotlib.pyplot as plt
