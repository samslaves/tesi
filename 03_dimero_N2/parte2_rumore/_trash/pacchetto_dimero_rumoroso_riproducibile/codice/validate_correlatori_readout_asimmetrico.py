...
RUOLO NELL'INSIEME DEL PACCHETTO: e' lo script di controllo per
l'estensione readout asimmetrico -- l'unico dei quattro validate_*.py
che verifica un risultato NON garantito dalla teoria (l'invarianza di
N* per readout asimmetrico e' un fatto empirico, non un teorema, a
differenza del caso simmetrico verificato in
validate_scan_parametri_rumore_dimero.py). E' anche il file che
stabilisce la robustezza del risultato ben oltre il caso illustrativo
citato nella documentazione, tramite lo stress test a rapporti estremi.
"""
import numpy as np
