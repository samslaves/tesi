...
\texttt{\seqsplit{simmetrie\_dmi\_spin.pdf}} & Simmetria/antisimmetria dell'Hamiltoniana di scambio fra due spin 1/2: operatore di permutazione $P_{12}$, componente simmetrica (Heisenberg) vs antisimmetrica (DM).\\
\bottomrule
\end{longtable}
...
\midrule
\endhead
\texttt{\seqsplit{dimer\_exact.py}} & Benchmark esatto (diagonalizzazione), autovalori analitici per il caso base $D=0$; la stessa funzione \texttt{dimer\_hamiltonian(b,J,D)} supporta anche $D\neq0$ -- sorgente unica del \texttt{SparsePauliOp} riusata da tutto ciò che segue (VQE, Trotter, correlazioni). Concettualmente precede il VQE, non ne fa parte: è il riferimento con cui il VQE viene validato, esattamente come \texttt{trimer\_chain\_exact.py} per la catena.\\
\bottomrule
\end{longtable}
...
\texttt{\seqsplit{scelta\_ansatz\_RBS\_vs\_W.md}} & \textbf{Contributo originale}: confronto esplicito RBS vs $W_{ij}(\theta)$ di Crippa et al. per il blocco $M$-conservante del PMA -- scelta del gate, indipendente dal valore di $D$.\\
\bottomrule
\end{longtable}
...
\texttt{\seqsplit{vqe\_dimer\_spiegato.ipynb}} & Guida interattiva a \texttt{vqe\_dimer.py}: perché il VQE dà un limite superiore, come l'estimator calcola $\langle H\rangle_{\boldsymbol\theta}$, differenza ansatz HA vs PMA -- culmina nella spiegazione del perché il PMA fallisce con $D=0.2$.\\[4pt]
...
\texttt{\seqsplit{confronto\_ansatz\_entangler\_note.md}} & Note di lettura del notebook precedente: distinzione fra fallimenti dovuti alla sola assenza di rottura di simmetria (già a $D=0$, $b/J>2$) e fallimenti specificamente indotti dal DM (vicino a $b/J=2$).\\[4pt]
\texttt{\seqsplit{risultati\_vqe\_test2.md}} & Ground state VQE al punto di lavoro "test 2" ($b/J=0.35$, $D/J=0.80$) -- il punto usato in \texttt{relazione\_test\_parametri.md} (Fase 4) e poi riusato come stato iniziale delle correlazioni dinamiche.\\[4pt]
\texttt{\seqsplit{vqe\_ground\_state\_test2.ipynb}} & Notebook che implementa il calcolo di \texttt{risultati\_vqe\_test2.md}.\\
\bottomrule
\end{longtable}
...
\midrule
\endhead
\texttt{\seqsplit{quantum\_simulation\_domande\_al\_relatore.md}} & Domande aperte sul task Trotter appena ricevuto (serve un qubit ausiliario per le correlazioni? quale stato iniziale?).\\[4pt]
...
\texttt{\seqsplit{quantum\_simulation\_dimero\_trotter\_note.md}} & Note di convenzione: ordinamento sito$\to$qubit (Famiglia 2, ereditata dall'esempio del corso), convenzioni di segno delle frequenze.\\[4pt]
...
\texttt{\seqsplit{costo\_gate\_trotter.md}} & Conteggio dei gate per passo di Trotter e per $N$ passi, con la decomposizione \texttt{cx-rz-cx} al posto del gate nativo \texttt{RZZ}.\\[4pt]
\texttt{\seqsplit{circuito\_compatto\_teoria\_e\_limiti.md}} & Analisi collaterale su richiesta del relatore: un circuito più compatto (2 CNOT invece di 8 gate a due qubit) è possibile in teoria, ma perché non si usa per gli scopi di questo progetto -- verifica numerica con due metodi indipendenti.\\[4pt]
\texttt{\seqsplit{spiegazione\_shot\_rumore\_statistico.md}} & Spiegazione autosufficiente della soglia $1/\sqrt{N_{\rm shots}}$ per il rumore statistico, usata nei notebook Trotter in fase di esecuzione sul circuito.\\
\bottomrule
\end{longtable}
