...
def classical_exact(i, alpha, j, beta, t, J, Jp, b, D, psi0, H):
    ...
    Copiata da validate_circuito_correlazioni.py (04) invece che importata,
    per evitare il side-effect di chdir che quello script esegue come
    script (non e un modulo pulito)."""
    A = site_op(i, alpha)
    B = site_op(j, beta)
    Ut = sla.expm(-1j * H * t)
    Bpsi = B @ psi0
...
fig.tight_layout()
fig.savefig('figure/fig_trotter_trimero.png', dpi=170)
plt.close(fig)
...
fig.tight_layout()
fig.savefig('figure/fig_validazione_trimero.png', dpi=170)
plt.close(fig)
...
ax.grid(alpha=0.3)
fig.tight_layout()
fig.savefig('figure/fig_sito3_zero_trimero.png', dpi=170)
plt.close(fig)
