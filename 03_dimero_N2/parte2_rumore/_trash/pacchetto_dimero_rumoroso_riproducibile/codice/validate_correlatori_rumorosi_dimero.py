...
RUOLO NELL'INSIEME DEL PACCHETTO: e' lo script di controllo dedicato a
correlatori_rumorosi_dimero.py -- non definisce funzioni proprie (e'
uno script lineare, eseguito dall'alto in basso), ma e' il file che
certifica che quel modulo sia coerente sia con la Parte 1 (limite di
rumore nullo) sia con un percorso di calcolo indipendente (Monte Carlo)
per la formula di readout. E' anche il file che ha permesso di scoprire
e poi richiudere la regressione di circuito_correlazioni_dimero.py
(il supporto ansatz_params tornato temporaneamente a un placeholder):
senza il controllo 1 di questo script, quella regressione sarebbe
passata inosservata.
"""
import numpy as np
