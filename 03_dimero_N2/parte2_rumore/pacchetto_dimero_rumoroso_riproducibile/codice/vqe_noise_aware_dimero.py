...
RUOLO NELL'INSIEME DEL PACCHETTO: e' il modulo piu' esteso delle due
estensioni facoltative -- non si limita a rifare l'ottimizzazione VQE
sotto rumore, ma include anche i controlli diretti sugli stadi a valle
(Trotter, correlatori) e la verifica di generalita' a un secondo punto
di lavoro fisico, tutti riuniti qui invece che sparsi in piu' file.
Importa da vqe_dm_rumoroso_dimero.py (la versione "originale" con cui
confrontarsi) e da trotter_rumoroso_dimero.py/correlatori_rumorosi_dimero.py
(per i controlli a valle), rendendolo il modulo con piu' dipendenze
interne di tutto il pacchetto.
"""
import numpy as np
...
def transpila_dopo_assegnazione(x, nparam=3, optimization_level=3, seed_transpiler=7):
    ...
    Ruolo nel modulo: e' la primitiva usata da energia_rumorosa() ed
    energia_fedelta_rumorosa() per costruire il circuito da simulare --
    concentra in un solo punto la scelta "transpila dopo, non prima"
    discussa a lungo nel docstring di modulo.
    Ruolo nell'insieme: e' la versione, per l'ottimizzazione VQE
    noise-aware, dello stesso principio di transpilazione a blocchi gia'
    visto in trotter_rumoroso_dimero.py -- ma qui la direzione della
    trappola e' opposta (transpilare troppo presto, non troppo tardi).
    """
...
def energia_rumorosa(x, H, noise_model):
    ...
    Ruolo nel modulo: e' la funzione obiettivo minimizzata da
    vqe_noise_aware() -- ad ogni chiamata durante l'ottimizzazione,
    ricostruisce il circuito da zero con transpila_dopo_assegnazione()
    e ne calcola l'energia sotto rumore.
    Ruolo nell'insieme: e' l'analogo "solo energia" (senza fedelta') di
    vqe_energia_fedelta_rumorosa() in vqe_dm_rumoroso_dimero.py -- usata
    qui perche' durante l'ottimizzazione non serve la fedelta' ad ogni
    passo, solo alla fine.
    """
...
def energia_fedelta_rumorosa(x, H, noise_model, psi_exact):
    """Energia, fedelta' e conteggio CNOT del circuito VQE (parametri x)
    sotto rumore, con H passata esplicitamente come argomento.

    Ruolo nel modulo: e' la funzione di VALUTAZIONE finale (non usata
    durante l'ottimizzazione, che chiama invece energia_rumorosa) --
    usata nel blocco __main__ per riportare il risultato completo una
    volta trovato l'ottimo.
    Ruolo nell'insieme: a differenza di vqe_energia_fedelta_rumorosa()
    in vqe_dm_rumoroso_dimero.py (che legge b,J,D come costanti di
    modulo), questa funzione prende H come parametro esplicito -- e'
    proprio questa differenza di design che la rende sicura da usare a
    un punto di lavoro diverso da "test 2" (vedi
    verifica_secondo_punto_lavoro() piu' sotto, e GUIDA_USO.md sez. 6).
    """
    tqc = transpila_dopo_assegnazione(x)
...
def vqe_noise_aware(noise_model, R=6, maxiter=300, seed=0, x_seme=None):
    ...
    Ruolo nel modulo: e' la funzione "titolo" del file -- quella che
    risponde direttamente alla domanda posta nel docstring di modulo
    (l'ottimizzatore converge a parametri diversi se il rumore e' acceso
    fin dall'inizio?).
    Ruolo nell'insieme: e' l'analogo, sotto rumore, di vqe_multistart()
    in vqe_test2.py -- stessa struttura (multistart COBYLA + polish
    L-BFGS-B), ma con l'obiettivo valutato su un simulatore rumoroso
    invece che su uno statevector esatto. ATTENZIONE: come
    vqe_energia_fedelta_rumorosa(), costruisce H internamente dalle
    costanti di modulo J,b,D -- per un punto di lavoro diverso vanno
    sovrascritte esplicitamente PRIMA della chiamata (vedi
    GUIDA_USO.md sez. 6 per la spiegazione completa e un esempio).
    """
...
def controllo_invarianza_cnot(n_prove=5, seed=0):
    ...
    Ruolo nel modulo: e' il primo controllo eseguito nel blocco __main__
    -- se il conteggio CNOT dipendesse da theta, l'argomento fisico
    dell'intera estensione (canale isotropo => nessun vantaggio a
    rioptimizzare) perderebbe uno dei suoi due pilastri.
    Ruolo nell'insieme: e' richiamato anche da
    validate_vqe_noise_aware_dimero.py come primo dei controlli
    obbligatori -- una premessa verificata separatamente dal risultato
    principale, non solo assunta.
    """
    rng = np.random.default_rng(seed)
    conteggi = []
    dettagli = []
...
def controllo_N_star_correlatore(vqe_params_ideali, x_noise_aware, noise_model,
    ...
    Ruolo nel modulo: e' il controllo che chiude il "buco" lasciato dallo
    scope originale (che si fermava al solo Trotter) -- verifica sulla
    metrica dei correlatori invece di estendere per analogia un
    risultato dimostrato altrove.
    Ruolo nell'insieme: e' importato anche da
    validate_vqe_noise_aware_dimero.py e chiamato direttamente nel
    blocco __main__ di questo file -- il risultato (N* invariato) e' uno
    dei due controlli a valle citati nella documentazione dei risultati,
    insieme a quello sul Trotter.
    """
...
def verifica_secondo_punto_lavoro(noise_model, R=6, maxiter=300, seed=0,
    ...
    Ruolo nel modulo: e' la funzione piu' autonoma del file -- rifa da
    zero (diagonalizzazione esatta, multistart VQE ideale, VQE
    noise-aware, controlli a valle) l'intera catena logica del modulo,
    ma a un punto diverso, per rispondere alla domanda "e' un caso
    isolato di questo specifico punto di lavoro?".
    Ruolo nell'insieme: e' l'esempio di riferimento, in questo pacchetto,
    di come gestire correttamente funzioni che altrove leggono b,D,J da
    costanti di modulo (vqe_noise_aware, energia_rumorosa indiretta) --
    usa import locali dei moduli trotter_rumoroso_dimero e
    correlatori_rumorosi_dimero, sovrascrivendone temporaneamente gli
    attributi con salvataggio/ripristino esplicito (blocchi try/finally
    qui sotto), invece di assumere che i default vadano bene.
    """
    from dimer_exact import dimer_hamiltonian
