...
questo pacchetto, e viene importato da quasi ovunque (VQE ideale al
punto "test 2", benchmark VQE ansatz HA-vs-PMA, verifica dei
correlatori) per costruire l'Hamiltoniana e per fornire il riferimento
esatto (autovalori analitici, diagonalizzazione numerica) contro cui
ogni altro risultato viene confrontato. Se questo modulo cambiasse
silenziosamente, l'errore si propagherebbe a tutta la pipeline senza
dare nessun segnale esplicito -- per questo il self-test qui sotto
confronta i due metodi (numerico contro
