...
def _transpile_once(qc, optimization_level=3, seed_transpiler=7):
    """Transpila un circuito in base {rz,sx,x,cx} con parametri fissi
    (seed incluso, per riproducibilita' bit-per-bit fra sessioni).

    RUOLO NEL MODULO: utility comune ai quattro blocchi locali costruiti
    sotto. RUOLO NELL'INSIEME: implementa la strategia di transpilazione
    a blocchi (mai l'intero circuito) gia' fissata allo Stadio 3."""
...
def _prep_block(ansatz_params):
    """Blocco di preparazione: ansatz W-2q.6 (Parte 1) con i parametri
    VQE assegnati, transpilato una volta.

    RUOLO NEL MODULO: primo dei quattro blocchi composti in
    build_noisy_correlator_circuit. RUOLO NELL'INSIEME: stesso ansatz
    gia' usato allo Stadio 2 (vqe_dm_rumoroso_trimero_anello.py) --
    qui il circuito, non solo l'energia, viene riusato per intero."""
