...
\section{Il sistema aperto: sette documenti in più, stessa catena}

I quattro documenti precedenti restano a sistema chiuso: stato puro, evoluzione
unitaria, nessun errore di gate, nessuna misura imperfetta. Una volta chiusa
quella parte, il passo naturale è chiedersi cosa succeda alla stessa catena ---
preparazione, evoluzione, misura dei correlatori --- quando il sistema è
accoppiato a un ambiente che non si osserva, come su un dispositivo quantistico
vero. Non cambia l'Hamiltoniana: cambia il modo in cui il circuito viene
eseguito, e lo stato smette di essere un vettore puro $\ket\psi$ per diventare
un operatore densità $\rho$.

Sette documenti nuovi raccontano questo lavoro, nello stesso ordine in cui è
stato fatto e con lo stesso registro dei primi quattro. I primi cinque
ripercorrono la stessa catena di stadi, uno alla volta:
...
\toprule
\textbf{Doc.} & \textbf{Argomento} & \textbf{In una riga} \\
\midrule
5 & Il modello di rumore & due canali (depolarizzante 1q/2q, readout
   simmetrico), calibrati su un chip reale (\texttt{ibm\_torino}) \\
6 & VQE sotto rumore & quanto degrada uno stato fondamentale ideale eseguito
   su un simulatore rumoroso \\
7 & Dinamica sotto rumore & il compromesso fra errore di Trotter e rumore
   accumulato: esiste un numero di passi ottimale $N^*$ \\
8 & Correlatori sotto rumore & lo stesso test di Hadamard del Documento 4,
   ora con una misura reale e il readout incluso \\
9 & Scan sui parametri di rumore & come $N^*$ si sposta al variare del
   canale, e perché non dipende dal readout simmetrico \\
\bottomrule
\end{tabular}
\end{center}

A questi si aggiungono due estensioni facoltative, ciascuna su un argomento a
sé che non condivide fisica con l'altra --- una riguarda la strategia di
ottimizzazione, l'altra un'assunzione diversa nel modello di rumore, e per
questo restano due documenti separati invece di un unico capitolo:
...
\textbf{Doc.} & \textbf{Argomento} & \textbf{In una riga} \\
\midrule
10 & VQE noise-aware & rioptimizzare vedendo il rumore, invece di riusare i
   parametri ideali, non cambia nulla di misurabile --- verificato, non solo
   argomentato \\
11 & Readout asimmetrico & la richiesta opzionale del relatore: con
   $p_{01}\neq p_{10}$ l'invarianza di $N^*$ non è più garantita dalla teoria,
   ed è verificata numericamente fino a un caso estremo \\
\bottomrule
\end{tabular}
\end{center}

\messaggio{La differenza più importante fra queste due estensioni non è
l'argomento, ma il tipo di garanzia: nel Documento 10 l'invarianza discende da
un argomento di continuità reso quantitativo; nel Documento 11 il caso
simmetrico dava un'invarianza \emph{dimostrata analiticamente} (Documento 9),
che il caso asimmetrico non eredita automaticamente --- lì la risposta è
empirica, non un teorema.}
...
Per onestà di scope, due esclusioni esplicite:

\begin{itemize}[leftmargin=1.5em,itemsep=2pt]
\item \textbf{Il rumore.} Tutti i risultati dei Documenti 1--4 sono a
simulazione ideale: vettore di stato esatto, nessun errore di gate, nessun
conteggio finito. Il sistema aperto è trattato nei Documenti 5--11, appena
introdotti sopra.
