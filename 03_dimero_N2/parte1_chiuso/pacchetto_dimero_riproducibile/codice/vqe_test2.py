...
dimer_exact.py) del punto di lavoro "test 2" -- fornisce sia l'ansatz
variazionale canonico (pma_2q, rbs_block) sia la routine di
ottimizzazione (vqe_multistart) usati per produrre i parametri che poi
alimentano il modulo dei correlatori (circuito_correlazioni_dimero.py,
via ansatz_params). E' un ansatz diverso e piu' semplice di quelli
confrontati in vqe_dimer.py (HA, PMA a K layer): qui e' gia' la scelta
finale (PMA-2q.3), non un'alternativa messa a paragone. Nota
...
def pma_2q(nparam=3):
    ...
    Ruolo nell'insieme: e' la scelta finale di ansatz per il dimero
    (motivata dal confronto HA-vs-PMA di vqe_dimer.py, qui applicata al
    punto di lavoro specifico "test 2") -- circuito_correlazioni_dimero.py
    la importa da qui (non una propria copia) quando prepara lo stato
    tramite ansatz_params invece delle ampiezze esatte.
    """
...
def vqe_multistart(ansatz, H_op, w, v, R=6, maxiter=300, seed=0):
    ...
    Ruolo nell'insieme: e' la routine di ottimizzazione usata per
    produrre i parametri VQE finali di questo pacchetto (salvati in
    dati/ground_state_test2.npz da 01_genera_dati_vqe_ideale.py) --
    concettualmente analoga a run_vqe() di vqe_dimer.py, ma specializzata
    su un solo ansatz (PMA-2q.3) e un solo punto di lavoro, invece di
    confrontare piu' ansatz su uno sweep di campo.
