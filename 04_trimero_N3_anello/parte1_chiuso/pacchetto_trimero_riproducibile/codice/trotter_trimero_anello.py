...
def _Jij(bond, J, Jp):
    """Restituisce l'accoppiamento di scambio per un legame: J se il
    legame e' la base (1,2), J' altrimenti (lati (2,3) e (3,1) --
    triangolo isoscele).

    RUOLO NEL MODULO: helper interno usato da step_Hex per il coeff.
    corretto per legame. RUOLO NELL'INSIEME: nessun uso fuori da questo
    modulo."""
...
def U_exact(J, Jp, b, D, t):
    """Evoluzione unitaria esatta e^{-iHt} (nessun Trotter), per
    confronto col circuito.

    RUOLO NEL MODULO: benchmark per il self-test di convergenza
    (self-test 3). RUOLO NELL'INSIEME: stessa Hamiltoniana (H_parts)
    riusata in Parte 2 per lo stato bersaglio del compromesso
    Trotter/rumore (Stadio 3, target_state)."""
    H0, H_DM = H_parts(J, Jp, b, D)
    return sla.expm(-1j * (H0 + H_DM) * t)


def sz_tot(psi):
    """Magnetizzazione totale <Sz_tot> di uno stato (convenzione
    "diretta", Z1+Z2+Z3, non /2 come l'operatore di spin fisico).

    RUOLO NEL MODULO: osservabile usato nel self-test di convergenza.
    RUOLO NELL'INSIEME: nessun uso fuori da questo modulo nella
    pipeline attuale."""
...
# ---------------------------------------------------------------------
def _self_test():
    """Batteria di quattro self-test del modulo: (1) coerenza di
    SZ_TOT, (2) equivalenza circuito/matrice per un passo, (3)
    convergenza N->infinito del circuito Trotter verso U_exact, (4)
    fattorizzazione esatta di H0=campo+scambio (indipendente da
    Trotter).

    RUOLO NEL MODULO: unico contenuto del blocco __main__. RUOLO
    NELL'INSIEME: eseguito come script 03 nel pacchetto riproducibile
    di Parte 1 -- unica verifica di questo modulo, nessun file di
    output (solo resoconto a schermo)."""
...
def _bond_exchange_matrix(i, j):
    """Matrice 8x8 dello scambio isotropo XX+YY+ZZ sul legame (i,j),
    identita' sul terzo qubit.

    RUOLO NEL MODULO: usato solo dal self-test 2 (equivalenza
    circuito/matrice). RUOLO NELL'INSIEME: nessun uso fuori da questo
    modulo -- H_parts calcola l'Hamiltoniana completa diversamente
    (via trimer_ring_exact), questa e' una via di calcolo indipendente
    usata solo per la cross-verifica."""
...
def _bond_dm_matrix(i, j):
    """Matrice 8x8 del termine DM (XZ-ZX) sul legame (i,j), identita'
    sul terzo qubit.

    RUOLO NEL MODULO: usato solo dal self-test 2. RUOLO NELL'INSIEME:
    stessa nota di _bond_exchange_matrix -- via di calcolo indipendente
    per la cross-verifica, non riusata altrove."""
    from qiskit.quantum_info import Pauli
    pos = {1: 0, 2: 1, 3: 2}
    c1, c2 = ['I', 'I', 'I'], ['I', 'I', 'I']
