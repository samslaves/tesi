...
D,\ part,\ psi0,\ measure,\ ansatz\_params)} — il circuito completo: preparazione sul
...
rotazione finale ($H$ per Re, $R_x(\pi/2)$ per Im) ed eventuale misura. La
preparazione è selezionabile: \texttt{ansatz\_params=None} (default) usa
\texttt{prepare\_state} con le ampiezze esatte; \texttt{ansatz\_params=<6 parametri>}
usa il circuito VQE reale \texttt{W-2q.6} (vedi sotto).
\item \texttt{ancilla\_z\_expectation(qc)} — lettura esatta di
$\langle\sigma_z^{anc}\rangle$ da statevector (nessun rumore di shot).
\item \texttt{correlator\_from\_circuit(...,\ ansatz\_params)} — esegue entrambe le
parti (Re/Im) e combina il risultato complesso; propaga \texttt{ansatz\_params}.
\end{itemize}

\textbf{Come si usa.} Libreria, non notebook: si importano le funzioni necessarie.
Richiede \texttt{\seqsplit{trimer\_ring\_exact.py}},
\texttt{\seqsplit{trotter\_trimero\_anello.py}} e
\texttt{\seqsplit{vqe\_w2q6\_trimero\_anello.py}} nella stessa cartella.
...
\textbf{Conclusione.} Modulo maturo, riusato senza modifiche (poi esteso con
l'opzione \texttt{ansatz\_params}, vedi sotto) da tutti gli script e i notebook
successivi della fase.

\section*{\texttt{\seqsplit{vqe\_w2q6\_trimero\_anello.py}} + \texttt{\seqsplit{validate\_vqe\_circuito\_correlazioni.py}}}

Chiudono la pipeline VQE$\to$correlazioni con il circuito effettivo, sostituendo
lo stand-in a ampiezze esatte usato fino a questo punto: i file
\texttt{\seqsplit{vqe\_trimer\_ring\_W.py}} \& co.\ descritti in
\texttt{\seqsplit{trimero\_anello\_vqe\_dm.tex}} non esistono come moduli
indipendenti (il codice dell'ansatz vive nei notebook
\texttt{\seqsplit{confronto\_ansatz\_entangler\_trimero\_anello.ipynb}} /
\texttt{\seqsplit{analisi\_espressivita\_PMA\_anello.ipynb}}); qui la costruzione
è isolata in un modulo minimo e riusabile.

\textbf{Funzioni interne (\texttt{\seqsplit{vqe\_w2q6\_trimero\_anello.py}}).}
\begin{itemize}
\item \texttt{w\_block(qc,\ theta,\ q0,\ q1)} — blocco CNOT--$R_y(\theta)$--CNOT,
mirror diretto del PMA "originale" di Crippa et al.
\item \texttt{pma\_2q\_trimer\_exact(nparam,\ block)} — ansatz: $X$ sul qubit 0, poi tre
blocchi sui bond \texttt{BOND12=(2,1)}, \texttt{BOND23=(1,0)}, \texttt{BOND31=(0,2)},
poi $R_y$ indipendenti su tutti e tre i qubit finché non si esauriscono i parametri.
\texttt{W-2q.6 = pma\_2q\_trimer\_exact(6, w\_block)}.
\item \texttt{main()} — ottimizzazione a due stadi (multistart COBYLA, 12 partenze
casuali, seed fissato + polish L-BFGS-B) dell'energia di aspettazione
dell'Hamiltoniana \texttt{trimer\_hamiltonian\_dm} al punto di lavoro confermato
$J=1$, $J'=0.4$, $b=b_c=2.4$, $D=0.15$ (Opzione B); salva i 6 parametri ottimali in
\texttt{w2q6\_params\_optimal.npz}.
\end{itemize}

\textbf{Risultato.} Tutte e 12 le partenze convergono allo stesso punto:
$E_{\rm vqe}=-5.53437001739340$ contro $E_{\rm esatto}=-5.53437001739350$
($\Delta E=9.8\times10^{-14}$), fidelity $\mathcal F=0.99999999999961$ — coerente
col valore di cache (\texttt{\seqsplit{trimer\_ansatz\_sweep\_cache\_v2.json}},
voce \texttt{"DM|W-2q.6|2.4"}), che però non conservava il vettore dei parametri.

\textbf{Validazione (\texttt{\seqsplit{validate\_vqe\_circuito\_correlazioni.py}}).}
Confronto sistematico delle 81 combinazioni $C_{ij}^{\alpha\beta}(t)$ calcolate col
circuito a preparazione VQE reale contro quelle a preparazione esatta: errore medio
$3.0\times10^{-7}$, massimo $8.3\times10^{-7}$ (sul correlatore $C_{21}^{zz}$) —
dell'ordine di $\sqrt{1-\mathcal F}\sim6\times10^{-7}$, come atteso per un residuo di
fedeltà a livello di stato propagato agli osservabili dinamici.

\textbf{Conclusione.} La preparazione con l'ansatz VQE reale è validata: può
sostituire lo stand-in a ampiezze esatte ovunque nella fase (documentazione
aggiornata di conseguenza in
...
\section*{Quattro notebook della fase: \texttt{\seqsplit{correlazioni\_trimero\_anello\_simmetria.ipynb}}, \texttt{\seqsplit{correlazioni\_trimero\_anello\_esplorazione.ipynb}}, \texttt{\seqsplit{circuito\_correlazioni\_trimero\_anello\_tutte.ipynb}}, \texttt{\seqsplit{circuito\_correlazioni\_trimero\_anello\_vqe.ipynb}}}
...
\textbf{\texttt{\seqsplit{circuito\_correlazioni\_trimero\_anello\_vqe.ipynb}}} —
aggiunto in un secondo momento, dopo la chiusura della pipeline VQE$\to$correlazioni
(vedi la sezione su \texttt{\seqsplit{vqe\_w2q6\_trimero\_anello.py}} sopra). Mirror
completo del terzo notebook (stesse otto sezioni: misura diretta delle 81 combinazioni,
validazione rapida, heatmap d'insieme, vista d'insieme nel tempo su griglia $9\times9$,
selettore per ispezione singola, analisi escursione/simmetria/spettro, confronto con lo
scan a shot finiti, riepilogo), con un'unica differenza sistematica: la preparazione dello
stato usa ovunque il circuito VQE reale (\texttt{ansatz\_params}) al posto di
\texttt{prepare\_state}. Precede le otto sezioni una nona di ottimizzazione dell'ansatz
\texttt{W-2q.6} (stesso schema multistart COBYLA + polish). Lo scan a shot finiti della
Sez.~7 usa un file precomputato dedicato, \texttt{\seqsplit{scan81\_vqe\_trimero\_anello.py}}
(mirror di \texttt{\seqsplit{scan81\_trimero\_anello.py}}, stessa struttura, preparazione
VQE al posto di \texttt{prepare\_state}) $\to$ \texttt{scan81\_vqe\_results.npz/json}. A
differenza dei primi tre, non è un mirror del dimero: è specifico di questa fase del
trimero anello.

\textbf{Come si usano.} Notebook, eseguiti per intero; i primi tre richiedono i quattro
moduli sopra nella stessa cartella (e, per l'ultima sezione del terzo,
\texttt{scan81\_results.npz} già calcolato); il quarto richiede in più
\texttt{vqe\_w2q6\_trimero\_anello.py} e, per la sua Sez.~7,
\texttt{scan81\_vqe\_results.npz} (generato eseguendo prima
\texttt{\seqsplit{scan81\_vqe\_trimero\_anello.py}}, che a sua volta richiede
\texttt{w2q6\_params\_optimal.npz} già presente).

\textbf{Verifiche.} Tutte e ventotto le celle del quarto notebook (e tutte quelle dei
primi tre) eseguite senza errori (verificato iterando su tutti gli output di tutte le
celle); ogni sezione del quarto riproduce, entro l'errore di Trotter e statistico già
caratterizzati, gli stessi risultati numerici e la stessa lettura qualitativa delle
figure del terzo (heatmap con gli stessi quattro zeri strutturali evidenziati e la stessa
cella $C_{33}^{zz}$ dominante; stessa distinzione ricco/piatto nello spettro; rapporti di
simmetria $C_{11}/C_{22}$ tutti entro $\pm3\%$ dal valore atteso $\eta_\alpha\eta_\beta$).
Errore totale (Trotter+VQE) medio $8.81\times10^{-4}$, massimo $2.64\times10^{-3}$ —
indistinguibile, alla precisione riportata, da quello della versione a stato esatto.

\textbf{Conclusione.} I primi tre chiudono, per il trimero anello, il mirror dei tre
notebook della fase correlazioni già completata per il dimero; il quarto chiude la
pipeline VQE$\to$correlazioni con il circuito VQE reale in ogni sezione dell'analisi, non
solo nella misura di un caso isolato.
...
del sito 3 quasi congelata al punto di lavoro adottato. La pipeline VQE$\to$
correlazioni è stata infine chiusa con il circuito effettivo (ansatz \texttt{W-2q.6}),
sostituendo lo stand-in a ampiezze esatte usato fino a quel punto, con errore
residuo sulle correlazioni ($\sim10^{-7}$) coerente con la fedeltà dell'ansatz.
Aperti per il seguito: derivazione principiata (stile $R_1$ del dimero) del punto
Trotter dimostrativo $R_0$ del trimero; rumore di gate reale (Parte 2 della tesi);
Fase 5 (VQE con DM) per la catena aperta.
