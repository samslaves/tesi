...
2. Readout asimmetrico & senza modifiche concettuali & \textbf{chiuso} \\
...
\textbf{Estensione 2 chiusa, nessun ostacolo concettuale confermato}:
verificato direttamente dal codice (\texttt{circuito\_correlazioni\_trimero\_anello.py})
...
numero di qubit di registro (3 sull'anello contro 2 sul dimero). Metodologia
allineata al nuovo standard del dimero (formula analitica + cross-check
Monte Carlo con shot veri, sempre insieme). Unico punto che richiedeva
attenzione, verificato esplicitamente prima di scrivere il codice: la
diversa convenzione dei qubit (ancilla = qubit 3 sull'anello, primo
carattere della stringa di misura, non ultimo come sul dimero). Risultato:
$N^*=3$ invariato su limite simmetrico, split illustrativo ($3\times$,
cross-check Monte Carlo concorde) e stress test fino a $50\times$ ---
stesso esito qualitativo del dimero, nessuna sorpresa. Dettaglio completo
in \texttt{risultati\_readout\_asimmetrico\_trimero\_anello\_definitivo.tex}.

\messaggio{Entrambe le estensioni facoltative sull'anello sono ora chiuse.
L'estensione 1 (VQE noise-aware) ha rivelato un fenomeno nuovo rispetto al
dimero; l'estensione 2 (readout asimmetrico) si è chiusa esattamente come
previsto, senza sorprese.}
...
Estensione readout asimmetrico & chiusa & chiusa \\
\bottomrule
\end{tabular}
\end{table}
