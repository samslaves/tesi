...
Verifica obbligatoria di vqe_noise_aware_dimero.py.
  ...
  5. N* della quantum simulation (Trotter) sotto rumore, con i parametri
     ...
     parametri ideali.
  6-8. Controlli aggiuntivi dell'estensione: N* del correlatore
     (controllo diretto, non per analogia dal punto 5), generalita' su
     un secondo punto di lavoro fisico, e sull'intera griglia dello scan
     sui parametri di rumore -- vedi i rispettivi blocchi nel file.

RUOLO NELL'INSIEME DEL PACCHETTO: e' il piu' esteso dei quattro script
di validazione -- copre non solo vqe_noise_aware_dimero.py in isolamento
ma anche la sua interazione con trotter_rumoroso_dimero.py e
correlatori_rumorosi_dimero.py (i due stadi a valle su cui la domanda
"conviene ottimizzare di nuovo sotto rumore?" viene davvero verificata,
non solo la domanda sull'energia del solo VQE). E' il file che chiude,
punto per punto, ogni affermazione fatta nella documentazione dei
